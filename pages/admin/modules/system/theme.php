...
if (isset($_GET['customize'])) {
  ...
  if ($theme_type == 'public') {
    ...
    if (!file_exists($path)) {
      $path = AQUA_THEMES . $theme_dir . '/tinfo.inc.php';
    }
...
$plugin_theme_dir = new simbio_directory(AQUA_THEMES);
$plugin_theme_dir_tree = $plugin_theme_dir->getDirectoryTree(1);

$dir_tree = array_merge($plugin_theme_dir_tree, $dir_tree);

// sort array by index
ksort($dir_tree);

$base_template = SB.'admin/admin_template/';
$base_web_template = AWB . 'admin_template/';
...
foreach ($dir_tree as $dir) {
  $path = $base_template . $dir;
  
  if (is_dir($path)) {
    $preview = file_exists( $path . '/preview.jpg') ? 'preview.jpg' : 'preview.png';
    $preview = SWB.'admin/admin_template/'. $dir . '/'.$preview;
  } else if (is_dir($path = AQUA_THEMES . $dir)) {
    $preview = file_exists( $path . '/preview.jpg') ? 'preview.jpg' : 'preview.png';
    $preview = AQUA_WEB_BASE . 'themes/'. $dir . '/'. $preview;
  }
  ...
  $output .= '<img class="card-img-top rounded" src="' . $preview . '" height="150" />';
  $output .= '</div>';
  $output .= '<div class="card-footer border-0">' . $_btn . '</div>';
  $output .= '</div>';
  $output .= '</div>';
